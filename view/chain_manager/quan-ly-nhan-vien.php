...
$quanlyList = $mCuaHang->selectAllQuanLy();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['id'])) {
        ...
        $quanly = $mCuaHang->selectOneQL($id);
        ...
        if(!empty($quanly)) {
            if ($quanly['TrangThai'] == 1) {
                $mCuaHang->updateTrangThaiQL($id, 0);
            } else {
                $mCuaHang->updateTrangThaiQL($id, 1);
            }
        }
        ...
        <tbody id="ds-nhan-vien-tbody">
            ...
            if (!empty($quanlyList)) {
                foreach ($quanlyList as $ql) {
                    // Xác định trạng thái
                    $trangThai = ($ql['TrangThai'] == 0) ? 'Hoạt động' : 'Không hoạt động';

                    // Phân loại vào mảng tương ứng
                    if ($trangThai == 'Hoạt động') {
                        $quanlyHoatDong[] = $ql;
                    } else {
                        $quanlyNgungHoatDong[] = $ql;
                    }
                }
            }
            ...
            foreach ($quanlyHoatDong as $ql) {
            ?>
                <tr data-employee-id="<?= $ql['ID_QuanLyCuaHang'] ?>">
                    ...
                    <td><?php
                        if ($ql['PhanQuyen'] == 3) {
                            echo 'Nhân viên';
                        } else if ($ql['PhanQuyen'] == 4) {
                            echo 'Nhân viên bếp';
                        } else if ($ql['PhanQuyen'] == 2) {
                            echo 'Quản lý cửa hàng';
                        }
                        ?></td>
                    <td><?= $ql['TenCuaHang'] ?></td>
                    ...
                    <td style="width: 200px">
                        <div class="d-flex justify-content-center alighn-items-center">
                            <button data-bs-toggle="modal" data-bs-target="#formEdit" style="margin-right: 10px"
                                type="button" class="btn btn-warning"
                                onclick="editEmployee('<?= $ql['ID_QuanLyCuaHang'] ?>')">Sửa</button>
                            <form action="" method="POST">
            ...
            if (!empty($nhanvienNgungHoatDong)) {
            ...
            if (!empty($quanlyNgungHoatDong)) {
                // In ra bảng các nhân viên ngưng hoạt động
                foreach ($quanlyNgungHoatDong as $ql) {
                ?>
                    <tr data-employee-id="<?= $ql['ID_QuanLyCuaHang'] ?>">
                        <td><?= $ql['ID_QuanLyCuaHang'] ?></td>
                        <td><?= $ql['HoTen'] ?></td>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        if (response.data.PhanQuyen != 2) {
                            $('#updateEmployeeID').val(response.data.ID_NhanVien);
                        } else {
                            $('#updateEmployeeID').val(response.data.ID_QuanLyCuaHang);
                        }
